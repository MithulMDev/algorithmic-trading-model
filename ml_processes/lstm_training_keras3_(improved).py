...
def save_model_and_results(model, history, metrics, feature_cols):
    """Save model, architecture, and results - BULLETPROOF VERSION"""
    log_progress("SAVING MODEL AND RESULTS")
    
    run_dir = CONFIG['run_dir']
    save_errors = []
    
    # PRIORITY 1: Save full model (MOST IMPORTANT - do this first!)
    try:
        ...
        # Try Keras 3.x format
        try:
            model_path = os.path.join(run_dir, 'final_model.keras')
            model.save(model_path)
            print(f"✓ Full model saved (Keras format): {model_path}")
        except Exception as e2:
            error_msg = f"✗ Error saving model (Keras format): {e2}"
            print(error_msg)
            save_errors.append(error_msg)
    
    # PRIORITY 2: Save model architecture as JSON
    try:
        ...
        error_msg = f"✗ Error saving architecture: {e}"
        print(error_msg)
        save_errors.append(error_msg)
    
    # PRIORITY 3: Save model weights separately (FIXED for Keras 3.x)
    try:
        # Detect Keras version and use appropriate extension
        import keras
        keras_version = keras.__version__
        
        if keras_version.startswith('3'):
            # Keras 3.x requires .weights.h5
        ...
        error_msg = f"✗ Error saving weights: {e}"
        print(error_msg)
        save_errors.append(error_msg)
    
    # PRIORITY 4: Save model summary
    try:
    ...
    # PRIORITY 6: Save feature columns
    try:
        ...
        error_msg = f"✗ Error saving features: {e}"
        print(error_msg)
        save_errors.append(error_msg)
    
    # PRIORITY 7: Save configuration
    try:
        ...
        for k, v in CONFIG.items():
            if isinstance(v, (str, int, float, bool, list, tuple)):
                ...
                config_safe[k] = 'None'
        config_df = pd.DataFrame([config_safe])
    ...
    # PRIORITY 8: Save training history
    try:
    ...
    # Report any errors
    if save_errors:
        print(f"\n⚠ WARNING: {len(save_errors)} error(s) occurred during saving:")
        for error in save_errors:
            print(f"  {error}")
        print(f"\n✓ However, the trained model was saved successfully!")
        print(f"  You can still use the model even if some auxiliary files failed.")
    else:
